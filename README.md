...
`order:submit` — переход к следующему шагу оформления.
...
`order:submit` — пользователь перешёл к следующему шагу заказа.

`contacts:submit` — пользователь завершил оформление заказа.
